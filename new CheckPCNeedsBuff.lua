...
function RGMercUtils.CheckPCNeedsBuff(spell, targetId, targetName)
	...
	elseif mq.TLO.DanNet(targetName)() ~= nil then
		...
		elseif spellResult == "NULL" then
			...
			if triggerCt >= numEffects then
			    RGMercsLogger.log_verbose("CheckPCNeedsBuff() DanNet found %d of %d existing triggers for %s(ID:%d) on %s, ending.", triggerCt, numEffects, spellName, spellID, targetName)
			    return false
		    end
